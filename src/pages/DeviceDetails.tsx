        ...
        {state.tabIndex === 1 && (
          <div className="tab-content page-content-with-sidebar">
            <form name="collectorNameForm" onSubmit={update}>
              ...
              <a className="btn btn-link" href="/a/kentik-connect-app?tab=device-list">
                Cancel
              </a>
            </form>
          </div>
        ...
        {state.tabIndex === 2 && (
          <div className="tab-content page-content-with-sidebar">
            <div className="plugin-markdown-readme">
              ...
              <p>
                Setting up a device to use with Kentik Detect involves configuration steps on the device itself and also
                on the Devices » Add Device page in the Kentik Detect portal. Before you start, you'll need to know
                whether you'll be exporting flow to the Kentik Detect servers directly or through a local
                encryptor/redirector running the Kentik software called "chfagent" (see NetFlow Proxy Agent).
              </p>

              <p>
                The device configuration process varies depending on device manufacturer, but is typically performed in
                "configuration mode" or in a "configuration editor." Before you start you'll need to know the following
                information:
                <ul>
                  <li>
                    The destination IP and Port for the router to send flow too (flow collector IP). Depending on your
                    configuration, this will be either the Kentik Detect servers (see the device tab for "flow collector
                    IP" and "flow collector port") or will be the IP and Port you chose on your local
                    encryptor/redirector running chfagent.
                  </li>
              ...
              <em>Notes: </em>
              <p>
                - Routers must be configured to persist SNMP interface IDs across reboots. The example configurations
                below include, where applicable, the commands to accomplish this.
              </p>
              <p>
                - Every vendor changes flow configuration slightly based on hardware and software versions. The
                configuration information below is provided for reference only. Check your router vendor documentation
                before configuring your devices for use with Kentik Detect.
              </p>

              <h2>SNMP OID Polling</h2>
              <p>
                OIDs are identifiers for SNMP objects that each represent the properties of a network-connected device
                such as a router. An OID takes the form of a path to the SNMP object it represents. Like a standard HTTP
                path, each segment represents a successively narrower slice of the entire networked universe, but in the
                case of an OID each segment is a pre-assigned number. The base OID for MIB-2 defined SNMP variables is
                1.3.6.1.2.1.
              </p>
              ...
              <p>
                ...
                <ul>
                  <li>
                    <strong>Selected counter OIDs</strong>: polled every two minutes; stored for SNMP-related querying,
                    e.g. comparison to flow (see Compare Flow with SNMP).
                  </li>
                  <li>
                    <strong>Selected info OIDs</strong>: polled every 30 minutes for interface information.
                  </li>
                </ul>
              </p>
              <p>
                ...
                <ul>
                  ...
                  <li>
                    Permit SNMP polling of the router from the IPs listed in the Device SNMP Polling IPs field of the
                    Device Details page in the portal.
                  </li>
                  <li>
                    Set community on the router to match the SNMP Community string indicated on the Device Details page
                    for the router.
                  </li>
                  <li>
                    If the router has been configured to block polling of any of the specific OIDs polled by Kentik
                    Detect, re-enable polling of those OIDs.
                  </li>
                </ul>
              </p>
              <p>
                If you've successfully completed the steps above, after about 30 minutes (one complete polling interval)
                ...
                <ul>
                  ...
                  <li>
                    Verify that blue bars for SNMP ingress and egress are present in the left-hand columns of the
                    Interfaces list.
                  </li>
                  <li>
                    Click the Traffic button to go to the Data Explorer, where you'll see a graph comparing SNMP and
                    flow rates for the router over the preceding hour.
                  </li>
                </ul>
              </p>
            </div>
